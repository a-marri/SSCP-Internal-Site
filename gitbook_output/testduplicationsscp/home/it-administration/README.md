...
HOST \<license\_server\_name or ip\_address> \<license-server-hostid#> \<main-rlm-server-port#>
...
There is a sscpftp user that owns all the files inside /vol0/ftp, but not /vol0/ftp itself. Since the sscpftp user has a public password, it has no shell access and is chrooted to /vol0/ftp. See the bottom of /etc/ssh/sshd\_config for details:
